...
const { TabPane } = Tabs,
  ...
  followedDetail = _.flatten(
    followedIds.map(id => data.questions.filter(question => question.id === id))
  ),
  createdIds = data.user.questions.created,
  createdDetail = _.flatten(
    createdIds.map(id => data.questions.filter(question => question.id === id))
  ),
