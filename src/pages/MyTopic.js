...
class MyTopic extends Component {
    ...
    getTopics = param => {
        ...
        topics.getTopics(param).then(respone => {
    ...
    postTopic = data => {
        ...
        newTopics.postTopic(data).then(respone => console.log("respone", respone));
    };

    patchTopic = (id, param) => {
        const editTopic = new Topics();
        editTopic.patchTopic(id, param).then(respone => console.log("respone", respone));
    };
    deleTopic = id => {
        const trashTopic = new Topics();
        trashTopic.deleTopic(id).then(respone => console.log("respone", respone));
    };

    onSearchClick() {
        const { searchValue, title } = this.state;
        const params = {
            sort: title,
            keyword: searchValue
        };
